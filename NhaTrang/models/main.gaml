...
global {
	...
	map<rgb, string> legends <- [color_car::"Cars", color_EVs::"ElecCars", color_road::"Roads", color_closed::"Closed Roads", color_lake::"Rivers & Lakes"];
	rgb color_car <- #lightblue;
	rgb color_EVs <- #cyan;
	rgb color_road <- #lightgray;
	rgb color_closed <- #mediumpurple;
	rgb color_lake <- rgb(165, 199, 238, 255);
	geometry shape <- envelope(shape_file(resources_dir + "output_shapefile.shp"));
	file road_shp <- shape_file(resources_dir + "line1.shp");
	file chargingstation_shp <- shape_file(resources_dir + "output_shapefile.shp");
	// Initialization 
	...
	init {
		...
		create charging_station from: chargingstation_shp;
